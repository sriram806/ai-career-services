...
export const sessions = pgTable(
  ...
  (table) => {
    return {
      ...
      refreshTokenHashUniqueIdx: uniqueIndex('sessions_refresh_token_hash_unique_idx').on(
...
export const emailVerificationTokens = pgTable(
  ...
  (table) => {
    return {
      tokenHashUniqueIdx: uniqueIndex('email_verification_tokens_hash_unique_idx').on(
        table.tokenHash,
      ),
...
export const trustedDevices = pgTable(
  ...
  (table) => {
    return {
      ...
      fingerprintIdx: uniqueIndex('trusted_devices_fingerprint_idx').on(
        table.userId,
...
export const oauthAccounts = pgTable(
  ...
  (table) => {
    return {
      providerUserIdx: uniqueIndex('oauth_accounts_provider_user_idx').on(
        table.provider,
...
export const connectedAccounts = pgTable(
  ...
  (table) => {
    return {
      userIdProviderIdx: uniqueIndex('connected_accounts_user_provider_idx').on(
        table.userId,
        table.provider,
      ),
    };
...
export const profileVersions = pgTable(
  ...
  (table) => {
    return {
      userIdVersionIdx: uniqueIndex('profile_versions_user_version_idx').on(
        table.userId,
        table.version,
      ),
    };
...
export const githubCredentials = pgTable(
  ...
  (table) => {
    return {
      userIdUniqueIdx: uniqueIndex('github_creds_user_id_unique_idx').on(table.userId),
    };
  },
);

// ─── GitHub Import Jobs Table ─────────────────────────
export const githubImportJobs = pgTable(
  'github_import_jobs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
    ...
    jobType: varchar('job_type', { length: 50 }).notNull().default('import'), // 'import', 'sync', 'refresh'
  ...
  (table) => {
    return {
      userIdIdx: index('github_jobs_user_id_idx').on(table.userId),
      statusIdx: index('github_jobs_status_idx').on(table.status),
    };
  },
);
...
export const githubSyncHistory = pgTable(
  'github_sync_history',
  {
    ...
    totalStarsCount: integer('total_stars_count').notNull().default(0),
  ...
  (table) => {
    return {
      userIdIdx: index('github_sync_user_id_idx').on(table.userId),
    };
  },
);
...
export const resumes = pgTable(
  'resumes',
  {
    ...
    userId: uuid('user_id')
      .references(() => users.id, { onDelete: 'cascade' })
      .notNull(),
    activeVersionId: uuid('active_version_id'), // Referenced manually to avoid circular schema dependencies in Drizzle
    status: varchar('status', { length: 50 }).notNull().default('uploaded'), // 'uploaded', 'queued', 'processing', 'completed', 'failed', 'cancelled'
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => {
    return {
      userIdUniqueIdx: uniqueIndex('resumes_user_id_unique_idx').on(table.userId),
...
export const resumeVersions = pgTable(
  'resume_versions',
  {
    ...
    fileHash: varchar('file_hash', { length: 64 }).notNull(), // SHA-256 duplicate checks
    source: varchar('source', { length: 50 }).notNull().default('upload'), // 'upload', 'reprocess', etc.
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => {
    return {
      ...
      hashIdx: index('resume_versions_hash_idx').on(table.fileHash),
    };
  },
);

// ─── Resume Upload Jobs Table ─────────────────────────
export const resumeUploadJobs = pgTable(
  'resume_upload_jobs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
  ...
  (table) => {
    return {
      userIdIdx: index('resume_upload_jobs_user_id_idx').on(table.userId),
      statusIdx: index('resume_upload_jobs_status_idx').on(table.status),
    };
  },
);

// ─── Resume Processing History Table ──────────────────
export const resumeProcessingHistory = pgTable(
  'resume_processing_history',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    resumeId: uuid('resume_id')
      .references(() => resumes.id, { onDelete: 'cascade' })
  ...
  (table) => {
    return {
      resumeIdIdx: index('resume_history_resume_id_idx').on(table.resumeId),
    };
  },
);
...
export const resumeEvents = pgTable(
  ...
  (table) => {
    return {
      resumeIdIdx: index('resume_events_resume_id_idx').on(table.resumeId),
      eventTypeIdx: index('resume_events_type_idx').on(table.eventType),
    };
  },
);

// ─── LinkedIn Import Jobs Table ──────────────────────
export const linkedinImportJobs = pgTable(
  ...
  (table) => {
    return {
      userIdIdx: index('linkedin_jobs_user_id_idx').on(table.userId),
      statusIdx: index('linkedin_jobs_status_idx').on(table.status),
    };
  },
);

// ─── Profile Merge Jobs Table ────────────────────────
export const profileMergeJobs = pgTable(
  'profile_merge_jobs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
  ...
  (table) => {
    return {
      userIdIdx: index('merge_jobs_user_id_idx').on(table.userId),
      statusIdx: index('merge_jobs_status_idx').on(table.status),
    };
  },
);
